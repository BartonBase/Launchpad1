...
| M-37 | decided; measured on devnet | The requester pays VRF at cost, separately (ADR-012). Devnet 2026-09-25 (real Switchboard, third-party reveal): commit 5,000, reveal 10,000 (2 signatures), oracle fee 0 lamports, so ≈ 20,000 lamports per draw with settle. Mainnet queue reward 5 lamports (read, not exercised). One-off reusable `init_randomness` setup: 8,229,760 at mainnet rent (LiteSVM) and 6,009,480 on devnet (same accounts at devnet's lower rent rate). Sigs are in DECISIONS CD Q2 | `real_switchboard_init_cost_breakdown`; devnet txs `2uwwVLZk…`, `2DWhZjXY…`, `4HTiwsxB…` |
| M-38 | partial | Mock graduation only under `test-mock-graduation` (built into `target/test-sbf`); production `.so` checked for the marker (0 hits); `deploy-devnet.sh` refuses if `mock_switchboard.so` is in `target/deploy`. The real Switchboard program (dumped from devnet `Aio4gaXj…`, devnet queue, state and oracle accounts as fixtures) runs our hand-built init/commit/recommit CPIs in LiteSVM and rejects a forged reveal with `InvalidSecpSignature`. On devnet (2026-09-25), with the production `.so` deployed by the allowlisted script, a gateway-signed reveal was settled by a third party. A tampered oracle signature is rejected (`SecpRecoverFailure` 6033, simulated) and a replayed reveal is rejected (2001). The vault's `init_randomness` CPI works against the live program. The deploy script stages an allowlist and checks markers (09f8b3a) | `vault/real_switchboard.rs` (4 tests), `scripts/test-deploy-guards.sh` |
